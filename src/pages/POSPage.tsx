...
import { Search as SearchIcon, TrendingUp, DollarSign, Users, AlertTriangle, RefreshCw, Check, X } from 'lucide-react';
...
export default function POSPage() {
  ...
  const [products, setProducts] = useState<Array<{id: string; name: string; sku: string; price: number; stock: number; barcode?: string}>>([]);
  ...
  // Handle barcode/scanner input (simulated)
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && e.currentTarget === barcodeRef.current) {
      e.preventDefault();
  ...
  const searchProducts = async () => {
    ...
    setIsLoading(true);
    try {
      // Mock product search - in real app this would be API call
      const mockProducts = [
        { id: '1', name: 'Organic Bananas', sku: 'BAN-ORG-001', price: 0.89, stock: 25, barcode: '0012345678905' },
        { id: '2', name: 'Premium Coffee Beans', sku: 'COF-PRE-005', price: 12.99, stock: 12, barcode: '0012345678912' },
        { id: '3', name: 'Whole Milk 2L', sku: 'MILK-WHL-002', price: 2.49, stock: 18, barcode: '0012345678929' },
        { id: '4', name: 'Whole Wheat Bread', sku: 'BREAD-WHT-003', price: 3.29, stock: 8, barcode: '0012345678936' },
        { id: '5', name: 'Free Range Eggs 12pk', sku: 'EGG-FRE-004', price: 4.99, stock: 30, barcode: '0012345678943' },
      ];
  ...
  // Add product by barcode (simulated)
      ...
      setCart(cart.map(item =>
        item.id === product.id
          ? {
              ...
              tax: (item.quantity + 1) * item.price * 0.2 // Assuming 20% VAT
            }
          : item
      ));
    } else {
  ...
  const holdCart = () => {
    ...
    toast({
      title: "Cart Held",
      description: "Current cart has been saved for later",
      variant: "default"
    });
    clearCart();
  };
            ...
            <div className="lg:col-span-2">
              <Card className="h-full">
                <CardHeader className="pb-4">
                  <h2 className="text-lg font-medium text-slate-900">Product Search</h2>
                  <div className="flex items-center space-x-2 mt-2">
                    <input
                      ...
                      className="flex-1 rounded border border-slate-300 px-3 py-2 pl-10 focus:outline-none focus:ring-2 focus:ring-[#0176D3] focus:border-transparent"
                    />
                            ...
                            <TableRow key={item.id}>
                              ...
                              <TableCell className="text-center">
                                <div className="flex items-center justify-center space-x-2">
                                  ...
                                  <span className="px-2 py-0.5 text-xs border border-slate-300 rounded">{item.quantity}</span>
                                  <button
                              ...
                              <TableCell className="text-center">
                                <button
                                  ...
                                  className="p-1 rounded hover:bg-[#FEE2E2] text-red-600 hover:bg-red-50"
                                >
                                  <X className="h-3 w-3" />
                                </button>
                              </TableCell>
          ...
          {cart.length > 0 ? (
            <div className="space-y-4">
              <div className="text-center">
                <h3 className="text-xl font-bold">OneStoreSolution</h3>
                <p className="text-sm text-slate-600">Retail Management System</p>
                <p className="text-xs text-slate-500">Receipt #{Date.now().toString().slice(-6)}</p>
              ...
              <div className="border-t border-dashed border-slate-200 pt-4 mb-2"></div>
...
// Import ShoppingCart icon (we'll define it inline for now)
function ShoppingCart() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M6 2L9 5l11 5H7"></path>
    </svg>
  );
}